...
# create integer day variable from date, starting at one
f$day <- as.integer(f$date - min(f$date)) + 1
f$day <- match(f$day, sort(unique(f$day)))
...
# fig 1: decline of 4231 over time--------------------

# overall frequencies over time: 4231 gets less common, replaced by first 433 and 442, then 433 gets more common than 442
# with day rather than date, no inter-season gaps
cbPalette <- c("#E69F00", "#56B4E9", "#009E73")

# create matrix containing proportion of each formation during a time bin (B days)
B <- 30
nbins <- round(max(f$day)/B) + 1
f$bin <- f$day %/% B + 1
plot_data <- matrix(NA, nbins, length(levels(f$formation)), dimnames = list(1:nbins,paste("f",levels(f$formation),sep="")))

for (i in 1:nbins) {
   plot_data[i,] <- table(f$formation[f$bin == i]) / sum(table(f$formation[f$bin == i]))
}
...
all_divs <- ggplot() +
  theme_cowplot() +
  ylim(0,0.6) +
  geom_line(data = plot_data[plot_data$formation == "4231" | plot_data$formation == "442" | plot_data$formation == "433",], aes(x = day, y = frequency, colour = formation), alpha = 0.8, lwd = 1.2) + theme(legend.position = c(0.8, 0.8)) +
  scale_colour_manual(values=cbPalette) +
  new_scale_colour() +
  geom_line(data = plot_data[plot_data$formation != "4231" & plot_data$formation != "442" & plot_data$formation != "433",], aes(x = day, y = frequency, group = formation), alpha = 0.2, lwd = 1, show.legend = F)

# by division
B <- 30
nbins <- round(max(f$day)/B) + 1
nformations <- length(levels(f$formation))+2
f$bin <- f$day %/% B + 1
plot_data <- matrix(NA, nbins*5, length(levels(f$formation))+2, dimnames = list(rep(1:nbins, each=5),c("division","bin",paste("f",levels(f$formation),sep=""))))
plot_data <- as.data.frame(plot_data)  # convert to dataframe
plot_data$division <- rep(1:5,nbins)
plot_data$bin <- rep(1:nbins, each=5)

for (i in 1:nbins) {
  for (j in 1:5) {
    plot_data[plot_data$bin == i & plot_data$division == j,3:nformations] <- table(f$formation[f$bin == i & f$division == levels(f$division)[j]]) / sum(table(f$formation[f$bin == i & f$division == levels(f$division)[j]]))
  }
}

plot_data <- pivot_longer(plot_data, -c(bin, division), names_to = "formation", values_to = "frequency", names_prefix = "f")  # convert from wide to long

plot_data$day <- plot_data$bin * B  # replace bin with day
plot_data$bin <- NULL
...
#  ylim(0,1) + 
  ...
  geom_line(data = plot_data[plot_data$formation == "4231" | plot_data$formation == "442" | plot_data$formation == "433",], aes(x = day, y = frequency, colour = formation), alpha = 0.8, lwd = 1, show.legend = F) +
  scale_colour_manual(values=cbPalette) +
  new_scale_colour() +
  geom_line(data = plot_data[plot_data$formation != "4231" & plot_data$formation != "442" & plot_data$formation != "433",], aes(x = day, y = frequency, group = formation), alpha = 0.2, lwd = 1, show.legend = F) +
  facet_grid(rows = vars(division))

plot_grid(all_divs, sep_divs, rel_widths = c(2,1))
ggsave2("figures/fig1_frequencies.png", width = 8, height = 5, dpi = 600)
...
# H1------
# the full model (m4) will be the best supported model as measured using WAIC, relative to personal-only, population-only and null models (m1, m2 and m3)
mcomparison <- compare(m1, m2, m3, m4)
mcomparison
...
saveRDS(mcomparison, file="modeloutputs/mcomparison.rds")
# mcomparison <- readRDS(file="modeloutputs/mcomparison.rds")
...
precis_m4 <- precis(m4, prob = 0.89, depth = 2)
precis_m4

saveRDS(precis_m4, file="modeloutputs/precis_m4.rds")
# precis_m4 <- readRDS(file="modeloutputs/precis_m4.rds")
...
# fig 2: plots of pers_use and pop_use, in interaction with win rates-------------

png("figures/fig2_pers_pop_interactions.png", width = 8, height = 4, units = 'in', res = 600)
...
text(0.3, 0.9, paste("+",W*100,"% performance", sep = ""), col = "chocolate1")
...
lines( b.seq , p_mu , col = "deepskyblue2", lwd = 2)
shade( p_ci , b.seq , col=col.alpha("deepskyblue2",0.2))
text(0.4, 0.9, paste("-",W*100,"% performance", sep = ""), col = "deepskyblue2")

par(xpd=NA)
text(-0.3,1.1,"B",cex=2)
...
post.r <- extract.samples(m4) # real data
...
precis_sigmadiffs <- precis(sigma_diffs)  # differences in all except population use & division
precis_sigmadiffs
plot(precis_sigmadiffs)

saveRDS(precis_sigmadiffs[,1:4], file="modeloutputs/precis_sigmadiffs.rds")
# precis_sigmadiffs <- readRDS(file="precis_sigmadiffs.rds")
...
precis_manager <- precis(m.manager)
precis_manager  # H4 not supported: very slightly positive b2 coefficient. Mostly no relationship.
#        mean   sd  5.5% 94.5% n_eff Rhat
# a      0.03 0.14 -0.19  0.26  1601    1
# b1    -0.16 0.18 -0.44  0.11  1469    1
# b2     0.10 0.07 -0.01  0.21  1757    1
# sigma  1.00 0.04  0.94  1.06  2092    1

saveRDS(precis_manager[,1:4], file="modeloutputs/precis_manager.rds")

# fig 3: win rate and use ratio------------
png("figures/fig3_ratio_winrate.png", width = 5, height = 5, units = 'in', res = 600)
...
plot( win_rate ~ info_ratio , xlab = "Manager population:personal information use", ylab = "Manager win rate")
lines( ratio.seq , mu.mean , lwd = 2)
...
# fig 4: plot of variation across divisions---------------

png("figures/fig4_divisions.png", width = 8, height = 4, units = 'in', res = 600)

par(mfrow=c(1,2))

# 4A: plot of pers_use in diff divisions
col_list <- c("gold2","chartreuse4","brown3","chocolate2","cornflowerblue")  # yellow, green, red, orange, blue
...
text(0.45,0.4,"Bundesliga", col = col_list[1])
text(0.6,0.5,"EPL", col = col_list[2])
text(0.75,0.6,"La Liga", col = col_list[3])
text(0.9,0.7,"Ligue 1", col = col_list[4])
text(0.2,0.9, "Serie A", col = col_list[5])

par(xpd=NA)
text(-0.3,1.1,"A",cex=2)
par(xpd=FALSE)

# plot variation across divisions

alpha <- 0.05
plot( NULL, xlim = c(0, 6), ylim = c(-2, 6), xlab = "reliance on personal info", ylab = "reliance on social info")
abline(h = 0, lty = 2)
abline(v = 0, lty = 2)
abline(a = 0, b = 1)
#points(apply(post$a_division[,,2], 1, mean) + post$b_pers_use, apply(post$a_division[,,3], 1, mean) + post$b_pop_use, col = col.alpha("black", alpha = alpha), pch = 20)  # means

for (j in 1:5) {
  points(post$a_division[,j,2] + post$b_pers_use, post$a_division[,j,3] + post$b_pop_use, col = col.alpha(col_list[j], alpha = alpha), pch = 20)
}
text(2.5,-1.5,"Bundesliga", col = col_list[1])
text(2.7,3.3,"EPL", col = col_list[2])
text(0.7,2.2,"La Liga", col = col_list[3])
text(2,4,"Ligue 1", col = col_list[4])
text(4.5,-0.5, "Serie A", col = col_list[5])

par(xpd=NA)
text(-1.8,6.6,"B",cex=2)

dev.off()

# explore between-manager variation in 4231 use
serieA_bymanager <- summarise( group_by(f[f$division == "Serie_A",], manager), mean = mean(fourtwothreeone) )
hist(serieA_bymanager$mean)

freq_bymanager <- summarise( group_by(f, manager, division), mean4231 = mean(fourtwothreeone) )
levels(freq_bymanager$division) <- c("Bundesliga", "EPL", "La Liga", "Ligue 1", "Serie A")

# 3 managers in Serie A used 4231 more than 50% of the time
sum(freq_bymanager$mean4231[freq_bymanager$division == "Serie A"] > 0.5)
# 10 more than 25%
sum(freq_bymanager$mean4231[freq_bymanager$division == "Serie A"] > 0.25)
# 35 never used it
sum(freq_bymanager$mean4231[freq_bymanager$division == "Serie A"] == 0)

# plot frequencies by division
ggplot( freq_bymanager, aes(mean4231)) +
  geom_histogram(binwidth = 0.05) +
  facet_grid(freq_bymanager$division) +
  theme_cowplot() +
  xlab("use of 4231 per manager")

# fig 5: plot of between manager variation-------------------------------

# list of managers and their win rates and number of games
View(summarise( group_by(f, manager), mean = mean(win), n = n() ))

# top 10 managers (level id) with >100 games ordered by win rate: Antonio Conte (25), Josep Guardiola (157), Carlo Ancelotti (40), Laurent Blanc (175), Massimiliano Allegri (206), Diego Simeone (72), Luis Enrique (185), Leonardo Jardim (176), Jose Mourinho (154), Arsene Wenger (28)

post <- extract.samples(m4)

png("figures/fig5_managers.png", width = 8, height = 4, units = 'in', res = 600)

par(mfrow=c(1,2))

textsize <- 0.7
alpha <- 0.03
plot( NULL, xlim = c(-5, 10), ylim = c(-10, 10), xlab = "reliance on personal info", ylab = "reliance on social info")
abline(h = 0, lty = 2)
abline(v = 0, lty = 2)
abline(a = 0, b = 1)

col_list <- c("gold2","chartreuse4","brown3","chocolate2","cornflowerblue")  # yellow, green, red, orange, blue
k <- c(25, 157, 40, 154, 28)  # conte, guardiola, ancelotti, mourinho, wenger
m.names <- levels(f$manager)[k[1]]
for (j in 1:5) {
  points(post$a_manager[,k[j],2] + post$b_pers_use, post$a_manager[,k[j],3] + post$b_pop_use, col = col.alpha(col_list[j], alpha = alpha), pch = 4)
  
  lines(ellipse(cor(post$a_manager[,k[j],2] + post$b_pers_use, post$a_manager[,k[j],3] + post$b_pop_use), scale = c(sd(post$a_manager[,k[j],2] + post$b_pers_use), sd(post$a_manager[,k[j],3] + post$b_pop_use)), centre = c(mean(post$a_manager[,k[j],2] + post$b_pers_use), mean(post$a_manager[,k[j],3] + post$b_pop_use)), level = 0.80), col = col.alpha(col_list[j], alpha = 1), lwd = 2)
}
text(-0.1,7.5, levels(f$manager)[k[1]], col = col_list[1], cex = textsize)
text(-2.9,4, levels(f$manager)[k[2]], col = col_list[2], cex = textsize)
text(4,-5.5, levels(f$manager)[k[3]], col = col_list[3], cex = textsize)
text(3,6.5, levels(f$manager)[k[4]], col = col_list[4], cex = textsize)
text(8.2,3, levels(f$manager)[k[5]], col = col_list[5], cex = textsize)
...
col_list <- c("gold2","chartreuse4","brown3","chocolate2","cornflowerblue")  # yellow, green, red, orange, blue
k <- c(264, 278, 179, 7, 198)  # donadoni, allardyce, alcaraz, pardew, hughes
m.names <- levels(f$manager)[k[1]]
for (j in 1:5) {
  points(post$a_manager[,k[j],2] + post$b_pers_use, post$a_manager[,k[j],3] + post$b_pop_use, col = col.alpha(col_list[j], alpha = alpha), pch = 4)
...
text(7.5,2, levels(f$manager)[k[1]], col = col_list[1], cex = textsize)
text(-0.5,-3.5, levels(f$manager)[k[2]], col = col_list[2], cex = textsize)
text(4.9,-3, levels(f$manager)[k[3]], col = col_list[3], cex = textsize)
text(-2.5,2.5, levels(f$manager)[k[4]], col = col_list[4], cex = textsize)
text(2.2,6.5, levels(f$manager)[k[5]], col = col_list[5], cex = textsize)
